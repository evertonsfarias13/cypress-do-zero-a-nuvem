describe("Central de Atendimento ao Cliente TAT", () => {
  ...
  it.only("exibe mensagem de erro quando o telefone se torna obrigatório mas nao é preenchido", () => {
    const longText = Cypress._.repeat("abcdefghijklmnopqrstuvxz", 10); //usando uma função repeat vai repetir 10x o texto
    //o proprio cypress tem algumas funcoes, usando o Cypress junto
    cy.get("#firstName").as("fieldname").type("Ev");
    cy.get("#lastName").as("fieldLastName").type("Farias");
  ...
  it('marca o tipo de atendimento "feddback" ', () => {
    cy.get('input[type="radio"][value="feedback"]')
      .check()
      .should("be.checked");
  });

  it('marca cada tipo de atendimento "feddback" ', () => {
    cy.get('input[type="radio"]') //ele vai pegar os 3 elementos radio
    .each((typeOfService) => {     //transformar todos elementos em um array e passar por cada um
      cy.wrap(typeOfService)      //empacota um a um , checa e valida se cada um foi chegado
      .check()
      .should("be.checked");
    })
  })

    it('marca ambos checkbox e depois desmarca o ultimmo', () => {
   ...
   // checou os dois checkbox, verificou se foi checkado, pegou o ultimo e deschecou
  })  
